...
function filterTerminalNoise(value) {
  ...
  const filtered = lines.filter((line) => {
    ...
    if (/codex\s+resume/i.test(text)) return false;
    if (/dangerously-bypass-approvals-and-sandbox/i.test(text)) return false;
    if (/--no-alt-screen/i.test(text)) return false;
...
function isLowSignalTitle(title, session) {
  ...
  if (!text) return true;
  if (GENERIC_TITLES.has(text.toLowerCase())) return true;
  if (TITLE_NOISE_PATTERNS.some((pattern) => pattern.test(text))) return true;
...
function pickRealTitle(messages, fallback, session) {
  ...
  if (firstUser?.text) return clampText(firstUser.text, 40);
  if (!isLowSignalTitle(fallback, session)) return clampText(fallback, 40);
  return clampText(fallback || workspaceName(session?.cwd), 40) || workspaceName(session?.cwd);
}
...
function pickPreview(messages, session, title) {
  ...
  return clampText(candidate, 72) || PREVIEW_FALLBACK[session?.kind] || '继续这个会话';
}

function fallbackTitleForSession(session) {
  if (!isLowSignalTitle(session?.name, session)) return clampText(session.name, 40);
  if (!isLowSignalTitle(session?.inputPreview, session)) return clampText(session.inputPreview, 40);
  if (session?.kind === 'history') return '历史会话';
  return clampText(session?.name || session?.providerLabel || '当前会话', 40) || '当前会话';
}

function fallbackPreviewForSession(session) {
  if (!isLowSignalTitle(session?.inputPreview, session)) return clampText(session.inputPreview, 72);
  if (!isLowSignalTitle(session?.name, session)) return clampText(session.name, 72);
...
function formatRelativeTime(value) {
  ...
  if (diff < week) return `${Math.max(1, Math.round(diff / day))} 天前`;
...
async function requestHistoryMessages(session) {
  if (!session?.resumeSessionId || historyApiAvailable === false) return null;
  const params = new URLSearchParams({ provider: session.provider, resumeSessionId: session.resumeSessionId });
  const response = await fetch(`/api/history-messages?${params.toString()}`, { credentials: 'same-origin' });
...
export function useSessionApp() {
  ...
  const groupedSessions = computed(() => {
    ...
    for (const session of state.sessions.map(decorateSession)) {
      if (!groups.has(session.groupName)) groups.set(session.groupName, []);
    ...
    return [...groups.entries()]
      ...
      .sort((a, b) => String(b.sessions[0]?.updatedAt || '').localeCompare(String(a.sessions[0]?.updatedAt || '')));
  });

  const activeSessionTitle = computed(() => {
    if (!state.activeSessionMeta) return '会话';
  ...
  function finalizeAssistantStream() {
    if (!state.activeStreamBuffer) return;
  ...
  async function hydrateSession(session, { includeMessages = false, silent = false } = {}) {
    if (!session || session.kind !== 'history' || !session.resumeSessionId) return null;
    const key = cacheKey(session);
    const cached = sessionCache[key];
    if (cached?.hydrated && (!includeMessages || cached.messages)) return cached;
    if (pendingHydrations.has(key)) {
      if (!includeMessages) return cached || null;
      while (pendingHydrations.has(key)) await wait(80);
    ...
    try {
      ...
      if (!silent) setStatus(error.message || String(error));
      return sessionCache[key] || null;
    } finally {
  ...
  async function handleLogin() {
    try {
      if (!state.accessToken) throw new Error('请输入 token');
      await request('/api/login', { method: 'POST', body: JSON.stringify({ token: state.accessToken }) });
  ...
  function attachLiveSocket(sessionId, historyMessages = []) {
    ...
    socket.addEventListener('message', (event) => {
      ...
      if (payload.type === 'exit') {
        finalizeAssistantStream();
        const exitMessage = createMessage('assistant', `会话已退出 (${payload.exitCode ?? 0})`, new Date().toISOString(), { source: 'system' });
  ...
  async function ensureLiveSession() {
    ...
    if (!state.activeSessionMeta) throw new Error('当前没有可继续的会话');
